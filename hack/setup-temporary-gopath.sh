...
function shim_gopath() {
  ...
  local TEMP_TEKTONCD="${TEMP_GOPATH}/src/github.com/lbernick"
  local TEMP_PIPELINE="${TEMP_TEKTONCD}/scheduler"
  local NEEDS_MOVE=1
...
function delete_pipeline_repo_symlink() {
  ...
  local TEMP_GOPATH="$(mktemp -d)/.gopath"
  if [ -d "$TEMP_GOPATH" ] ; then
    local REPO_SYMLINK="${TEMP_GOPATH}/src/github.com/lbernick/scheduler"
